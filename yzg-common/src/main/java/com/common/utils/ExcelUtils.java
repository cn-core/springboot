...
public class ExcelUtils {
    ...
    private static final short HEADER_BACKGROUND_COLOR = HSSFColor.PALE_BLUE.index;
    private static final short HEADER_FONT_COLOR = HSSFColor.DARK_RED.index;
    private static final short DEFAULT_ROW_HEIGHT = 500;
    private static final short DEFAULT_COLUMN_WIDTH = 20 * 256;

    private static final String FILE_NOT_FOUND_EXCEPTION = "路径指定的文件不存在!";
    private static final String BAD_FILE_FORMAT_EXCEPTION = "Excel文件格式不正确!";
    private static final String COLUMN_COUNT_NOT_SAME = "单行列数不一致!";
    private static final String THE_FILE_PATH_IS_EMPTY = "文件路径为空!";
     ...
     * @throws IOException
     */
    public static Sheet getExcelSheet(String path) throws IOException {
     ...
     * 获取Excel总行数
     */
    public static int rowCount(Sheet sheet) throws IOException {
        if (sheet == null) {
            throw new RuntimeException("无效的Sheet!");
        }
        // 获得总行数
     ...
     * 获取Excel总列数
     */
    public static int lineCount(Sheet sheet){
        if (sheet == null) {
            throw new RuntimeException("无效的Sheet!");
        }
        // 获得总列数
     ...
     * @throws Exception
     */
    public static String fileExists(String path) throws Exception {
        if (!StringUtils.isEmpty(path)) {
            File file = new File(path);
            if (!file.exists()) {
                throw new Exception(FILE_NOT_FOUND_EXCEPTION);
            }
        } else {
            throw new Exception(THE_FILE_PATH_IS_EMPTY);
        }
        return path;
    }
     ...
     * @param fileType {@link ExcelUtils#XLS}，{@link ExcelUtils#XLSX}
     * @param clazz 获得的对象类型{@link Class}
     * @param firstRowIndex 起始行索引，从0开始
     * @param lastRowIndex 结束行索引，从0开始
     */
     ...
     * @param lastColIndex 结束列索引，从0开始
     */
    public static String[][] xlsToStringArray(byte[] bytes, int fileType, int firstRowIndex, int lastRowIndex, int
            firstColIndex, int lastColIndex) {
        if (XLS != fileType && XLSX != fileType) {
            throw new RuntimeException(BAD_FILE_FORMAT_EXCEPTION);
        }
     ...
     * @author liyang
     */
    public static void export(String path, String sheetName, String[] headers, Collection<?> dataSet, String
            dateFormat) {
        OutputStream out = null;
        try {
    ...
    public static void export(String templatePath, String sheetName, int lineCnt, int colCnt,
                              int firstRowIndex, int firstColIndex, String[][] data, int decimal, boolean
                                      reserveHidden, String destPath) {
        // 复制模板到目的路径
    ...
    public static void export(String path, String templatePath, String sheetName,
                              int lineCnt, int colCnt, int firstRowIndex, int firstColIndex, String[][] data, int
                                      decimal) {
        Workbook workbook = null;
        OutputStream out = null;
        try {
     ...
     */
    public static void export(HttpServletResponse response, String filename, String encoding,
                              String templatePath, String sheetName, int lineCnt, int colCnt, int firstRowIndex, int
                                      firstColIndex,
    ...
    public static Object getCellValue(Cell cell, String type) {
        ...
        if (cellType == 0 && "java.lang.String".equals(type)) {
     ...
     * @throws IOException
     */
    private static void export(OutputStream out, String sheetName, String[] headers, String[][] data) throws
            IOException {
        Workbook workbook = null;
        try {
            // 声明一个工作薄
            ...
            for (int i = 0; i < headers.length; i++) {
                ...
                for (int j = 0; j < line.length; j++) {
                    ...
                    if (th.contains("^")) {    // 多行
                        ...
                        rowspan = StringUtils.isEmpty(span) ? 1 : Integer.parseInt(span);
                        if (rowspan > 1) {
                    ...
                    th = StringUtils.isEmpty(th) || th.contains("&nbsp;") ? "" : th;
                    ...
                    for (int i = 1; i < data.length; i++) {
                        ...
                        if (!curr.equals(old)) {
                            if (currIndex - oldIndex > 1) {
                                merge = new CellRangeAddress(oldIndex + headers.length, currIndex + headers.length -
                                        1, colIndex, colIndex);
                                mergeList.add(merge);
                            }
                            old = curr;
     ...
     * @param lastColIndex 结束列索引，从0开始
     */
    private static String[][] xlsToStringArray(Workbook workbook, int firstRowIndex, int lastRowIndex, int
            firstColIndex, int lastColIndex) {
